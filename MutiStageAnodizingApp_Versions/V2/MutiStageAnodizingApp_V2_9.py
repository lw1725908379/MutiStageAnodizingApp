...
class PowerSupply:
    ...
    def set_volt(self, V_input, error_range: float = 0.1, timeout: int = 30):
        """
        将电压设置为指定值，并等待电压稳定在目标值内。

        :param V_input: 目标电压（V）
        :param error_range: 允许的电压误差范围（V）
        ...
        while True:
            ...
            if abs(current_volt - V_input) <= error_range:
                logging.info(f"电压已稳定在 {current_volt} V")
                break
            if (time.time() - start_time) > timeout:
                logging.warning(f"电压设置超时: 当前电压为 {current_volt} V，目标电压为 {V_input} V")
                break
            time.sleep(0.5)  # 增加检查间隔
        elapsed_time = time.time() - start_time
        logging.info(f"电压设置完成，实际电压: {current_volt} V，用时 {elapsed_time:.2f} 秒")

    def set_volt_non_blocking(self, V_input):
        """
        将电压设置为指定值，不等待验证。
        ...
        """
        old_volt = self.V()
        logging.info(f"将目标电压从 {old_volt} V 设置为 {V_input} V (非阻塞)")
        self.V(V_input)
        logging.info(f"已发送电压设置命令：{V_input} V")
...
class ExperimentGUI:
    ...
    def on_closing(self):
        """处理窗口关闭事件，确保安全关闭电源并清理资源。"""
        if messagebox.askokcancel("退出", "您确定要退出程序吗？"):
            logging.info("程序关闭，开始执行清理操作。")

            # 启动一个新线程来处理关闭操作，避免阻塞主线程
    ...
    def cleanup_and_close(self):
        ...
        if self.serial_manager.power_supply:
            try:
                logging.info("禁用输出模式以确保安全。")
                self.serial_manager.power_supply.operative_mode(0)
                logging.info("输出模式已禁用。")
            except Exception as e:
                logging.error(f"禁用输出模式时发生错误: {e}")
        else:
            logging.warning("无法禁用输出模式，因为 power_supply 是 None。")

        # 然后尝试将电压设置为0
        if self.serial_manager.power_supply:
            try:
                logging.info("将电压设置为0 V以确保安全。")
                self.serial_manager.power_supply.set_volt(0)
                logging.info("已将电压设置为0 V。")
            except Exception as e:
                logging.error(f"设置电压为0时发生错误: {e}")
        else:
            logging.warning("无法设置电压为0，因为 power_supply 是 None。")

        # 如果实验正在运行，尝试停止它
        if self.experiment_controller and self.experiment_controller.is_experiment_running:
            logging.info("实验正在运行，尝试停止实验。")
